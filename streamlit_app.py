...
def main_app():
    """Main function to run the full Streamlit app."""
    ...
    if st.sidebar.button("Initialize Vector Store"):
        with st.sidebar.status("Loading documents and initializing vector store..."):
            ...
            try:
                ...
                st.sidebar.info("Vector store was created but LLM initialization failed. Try using the lightweight version.")
        ...
        if st.sidebar.button("Load Existing Vector Store"):
            with st.sidebar.status("Loading existing vector store..."):
                ...
                try:
                    ...
                    st.sidebar.error(f"Error initializing LLM: {str(e)}")
                    st.sidebar.info("Vector store was loaded but LLM initialization failed. Try using the lightweight version.")
